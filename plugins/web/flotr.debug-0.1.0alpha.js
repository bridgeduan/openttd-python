...
var Flotr = (function(){
	...
	function Plot(container, data, opts) {
		 ...
		 * 
		 * Parameters:
		 * 		object to fill flotr with
		 * 
		 * Returns:
		 * 		void
		 */
		function constructCanvas(parent){
			canvasWidth = target.getWidth();
			canvasHeight = target.getHeight();
            if (target.flotr){
                target.flotr.unbindEvents();
                target.flotr = null;
            }
            target.flotr = parent;
			target.innerHTML = '';
        ...
        /**
         * Function: (public) unbindEvents
         *
         *  Patched in by yorick to stop memory leaks
         *
         * Parameters:
         *      none
         * Returns:
        ...
        this.unbindEvents = function () {
            if(options.selection.mode != null){
                overlay.stopObserving('mousedown', mouseDownHandler);
            }
			overlay.stopObserving('mousemove', mouseMoveHandler);
			overlay.stopObserving('click', clickHandler);
        };
		/**
